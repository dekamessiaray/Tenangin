...
def render_field(key):
    ...
    if spec["type"] == "number":
        ...
        chosen_label = st.selectbox(
            label, display_labels, index=idx, key=f"w_{key}", help=spec.get("help"),
        )
        val = raw_values[display_labels.index(chosen_label)]
    elif spec["type"] == "radio":
        options = spec["options"]
        raw_values = [r for _, r in options]
        val = current if current in raw_values else raw_values[0]
        md(f'<p class="pill-toggle-label">{label}</p>')
        n = len(options)
        with st.container(key=f"toggle_{key}"):
            cols = st.columns([1] * n + [max(1, 4 - n)])
            for col, (disp, raw) in zip(cols[:n], options):
                with col:
                    is_selected = (val == raw)
                    if st.button(
                        disp, key=f"w_{key}_{raw}", use_container_width=True,
                        type="primary" if is_selected else "secondary",
                    ):
                        st.session_state.answers[key] = raw
                        st.rerun()
    else:
        ...
        with right:
            ...
            if agreement_score is not None:
                st.markdown(
                    ...
                    "Model Agreement"
                    "<span style='color:#94A3B8; font-weight:400;'> (bukan probabilitas kebenaran)</span>"
                    "</p>",
                    unsafe_allow_html=True,
                )
                st.progress(agreement_score)
                st.markdown(f"<p class='mono-num' style='font-size:0.95rem;'>{agreement_score*100:.1f}%</p>", unsafe_allow_html=True)
                st.caption(
                    "Model Agreement mengukur seberapa sepakat ketiga base learner "
                    "(XGBoost, CatBoost, LightGBM) terhadap hasil ini, dihitung dari "
                    "variasi probabilitas antar model - bukan tingkat keyakinan "
                    "statistik atas kebenaran prediksi."
                )
